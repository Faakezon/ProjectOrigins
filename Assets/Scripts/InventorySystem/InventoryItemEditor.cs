...
public class InventoryItemEditor : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        if (inventoryItemList != null)
        {
            ...
            if (inventoryItemList.itemList.Count > 0)
            {
                ...
                inventoryItemList.itemList[viewIndex - 1].itemDescription = EditorGUILayout.TextField("Item Desc", inventoryItemList.itemList[viewIndex - 1].itemDescription as string);
                ...
                GUILayout.BeginHorizontal();
                inventoryItemList.itemList[viewIndex - 1].itemType = (InventoryItem.ItemType)EditorGUILayout.EnumPopup("ItemType", inventoryItemList.itemList[viewIndex - 1].itemType, GUILayout.ExpandWidth(false));
                GUILayout.EndHorizontal();

                GUILayout.Space(10);

                switch (inventoryItemList.itemList[viewIndex - 1].itemType)
                {
                    case InventoryItem.ItemType.Weapon:
                        break;
                    case InventoryItem.ItemType.Armor:
                        inventoryItemList.itemList[viewIndex - 1].armorType = (InventoryItem.ArmorType)EditorGUILayout.EnumPopup("ArmorType", inventoryItemList.itemList[viewIndex - 1].armorType, GUILayout.ExpandWidth(false));
                        break;
                    case InventoryItem.ItemType.Consumable:
                        inventoryItemList.itemList[viewIndex - 1].HPRecovery = EditorGUILayout.IntField("HP Recovery", inventoryItemList.itemList[viewIndex - 1].HPRecovery, GUILayout.ExpandWidth(false));
                        break;
    ...
    void AddItem()
    {
        ...
        {
            itemName = "New Item",
            itemID = inventoryItemList.itemList.Count
        };
